...
test("Web and PWA alternate across an hour boundary with conflict, retry, correction, and projections", async ({ browser }) => {
  ...
  try {
    ...
    console.log(`[diag] ${new Date().toISOString()} caseId=${caseId} initialRevision=${revision}`)
    ...
    const webDrugText = await webDrug.text()
    console.log(`[diag] ${new Date().toISOString()} webDrug status=${webDrug.status()} body=${webDrugText}`)
    expect(webDrug.status(), webDrugText).toBe(200)
    revision = (JSON.parse(webDrugText) as { intraopRevision: number }).intraopRevision
    const revisionAfterWebDrugDirect = await currentRevision(web.request, caseId)
    console.log(`[diag] ${new Date().toISOString()} revisionFromWebDrugResponse=${revision} revisionFromFreshRead=${revisionAfterWebDrugDirect}`)
