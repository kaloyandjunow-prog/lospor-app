...
const INTERVAL  = 5
const ROW_COLS  = 6   // columns per row = 30 min per row
...
interface Props { startTime: string; endTime?: string; monitoring?: Record<string, boolean>; ibw?: number | null; tbw?: number | null; showAgentRow?: boolean; data: TimetableData; onChange: (d: TimetableData) => void; onEndCase?: () => void; onPostopContinued?: (items: string[]) => void; onInfusionTotals?: (totals: { name: string; total: number; unit: string }[]) => void; onFluidTotals?: (crystalloids: number, colloids: number, blood: number) => void }
...
function calcInfusionTotal(seg: TimetableInfusion): { amount: number; unit: string } {
            ...
            <div key={f.id} className="py-3 border-b border-slate-100 dark:border-[#2e2e2e] space-y-2">
              ...
              {d === "discontinue" && (
                <div className="flex items-center gap-2 pl-0.5">
                  <span className="text-[11px] text-slate-500 dark:text-slate-400">Amount infused:</span>
                  <input type="number" min={0} max={10000} step={50}
                    value={fluidAmounts[f.id] ?? ""}
                    onChange={e => setFluidAmounts(prev => ({ ...prev, [f.id]: e.target.value }))}
                    className="w-20 text-center text-sm border border-slate-200 dark:border-[#3a3a3a] rounded-lg px-2 py-0.5 bg-white dark:bg-[#2a2a2a] focus:outline-none focus:ring-1 focus:ring-blue-400"
                    placeholder="0" />
...
export function IntraopTimetable({ startTime, endTime, monitoring, ibw, tbw, showAgentRow = false, data, onChange, onEndCase, onPostopContinued, onInfusionTotals, onFluidTotals }: Props) {
  ...
  const [layout, setLayout] = useState<"expand" | "scroll">("expand")
  // End Case Modal
  ...
  // Close vitals popup on Enter (global listener — more reliable than portal bubbling)
  useEffect(() => {
    ...
    window.addEventListener("keydown", handleEnter, true) // capture phase — beats input handlers
  ...
  useEffect(() => {
    function tick() {
      ...
      if (diffSecs >= 0) {
        ...
        const needsExtend =
          (d.infusions ?? []).some(i => i.endCol < col && !i.stopped && true) ||
          (d.fluids    ?? []).some(f => f.endCol < col && !f.stopped) ||
          (d.agents    ?? []).some(a => a.endCol < col && !a.stopped)
        if (needsExtend) {
          oc({
            ...d,
            infusions: (d.infusions ?? []).map(i => i.endCol < col && !i.stopped && true ? { ...i, endCol: col } : i),
  ...
  function handleEndCaseConfirm(result: {
    ...
    for (const { id, amount, category } of result.discontinuedFluidWithAmounts) {
      extendFluid(id, col, true)
      if (category === "Crystalloids") crystalloids += amount
      else if (category === "Colloids") colloids += amount
      else if (category === "Blood products") blood += amount
    }
    onEndCase?.()
    ...
    if (crystalloids > 0 || colloids > 0 || blood > 0) onFluidTotals?.(crystalloids, colloids, blood)
  ...
  function renderRow(rowIdx: number, overrideColStart?: number, overrideColEnd?: number) {
    ...
    const rowW        = LABEL_W + rowCols.length * COL_W
    const rowNowPx    = isActiveRow && nowOffsetPx !== null ? nowOffsetPx - colStart * COL_W : null
    ...
    return (
      <div key={rowIdx} ref={isActiveRow ? activeRowRef : undefined}
        ...
        <div className="flex items-center justify-between px-3 py-1 bg-slate-50 dark:bg-[#1a1a1a] border-b border-slate-100 dark:border-[#2a2a2a]">
          ...
          {isActiveRow && <span className="inline-flex items-center gap-1 text-[9px] font-semibold text-orange-500 dark:text-orange-400"><span className="inline-block w-1.5 h-1.5 rounded-full bg-orange-500 animate-ping opacity-75" />LIVE</span>}
        </div>
        <div style={{ width: rowW, position: "relative" }}>

          {/* Now-line (only active row) */}
          {rowNowPx !== null && (
          ...
          {activeRows.map((row, ri) => (
            <div key={row.key} className={`flex items-center border-b border-slate-50 dark:border-[#222] ${ri % 2 === 1 ? "bg-slate-50/40 dark:bg-[#1a1a1a]/60" : ""}`}>
              <div style={{ width: LABEL_W, minWidth: LABEL_W }} className="flex flex-col items-end justify-center pr-2 py-1.5 gap-0 select-none">
          ...
          {showAgentRow && (() => {
            ...
            return (
              <div className="flex items-stretch border-b border-slate-200 dark:border-[#2e2e2e] bg-slate-50/60 dark:bg-[#1a1a1a]/60 relative" style={{ minHeight: 32 }}>
                ...
                {rowNowPx !== null && nowAgentSeg && (
                  <button type="button" title="Stop inhalation at current time"
                    onClick={e => { e.stopPropagation(); extendSegment(nowAgentSeg.startCol, nowCol!, true) }}
                    style={{ position:"absolute", left: LABEL_W + rowNowPx + 2, top:"50%", transform:"translateY(-50%)", zIndex:20 }}
                    className={stopBtnCls}>STOP</button>
                )}
                {rowCols.map(ci => {
                  ...
                  const labelMinW     = Math.max(0, (visEnd - visStart + 1) * COL_W - 16)
                  ...
                  return (
                    <div key={ci} style={{ width: COL_W, minWidth: COL_W }}
                          ...
                          <>
                            ...
                            {label && (
                              <span className={`absolute top-1/2 -translate-y-1/2 z-10 pointer-events-none select-none text-xs font-bold whitespace-nowrap flex items-center justify-center ${style2.text}`}
                                style={{ left: 0, width: (visEnd - visStart + 1) * COL_W }}>
                                {label}
                              </span>
                            )}
                          </>
                      ...
                      {ci !== nowCol && seg && !isDragPreview && !seg.stopped && (
                        <button type="button" title="Stop inhalation here" onClick={e => { e.stopPropagation(); extendSegment(seg.startCol, ci, true) }} className={hoverStopCls}>STOP</button>
                      )}
                      {!seg && !isDragPreview && (() => {
                  ...
                  {colDrugs.map(d => {
                    ...
                    return (
                      <div key={gi} draggable
                        ...
                        className={`flex items-start gap-1 rounded px-2.5 py-1.5 group cursor-grab active:cursor-grabbing transition-colors ${sel?.type === "drug" && sel.idx === gi ? "bg-violet-400 dark:bg-violet-600 ring-2 ring-violet-500 dark:ring-violet-400" : "bg-violet-100 dark:bg-violet-900/40 hover:bg-violet-200 dark:hover:bg-violet-800/40"}`}>
                            ...
                            {(isActualStart || isRowCont) && (() => {
                              ...
                              return (
                                <span className="absolute top-1/2 -translate-y-1/2 text-[10px] font-bold whitespace-nowrap pointer-events-none select-none text-center block"
                                  style={{ color, left: 0, width: (visEnd - visStart + 1) * COL_W }}>
                                  {seg.name}
                                </span>
                              )
                            })()}
          ...
          {computeFluidRows(data.fluids ?? []).map(({ label, segs, color }) => {
            ...
            return (
              <div key={label} className="flex min-h-[64px] border-t border-slate-100 dark:border-[#2a2a2a] relative">
                ...
                {rowNowPx !== null && nowFluidSeg && (
                  <button type="button" title="End fluid at current time"
                    onClick={e => { e.stopPropagation(); extendFluid(nowFluidSeg.id, nowCol!, true) }}
                    style={{ position:"absolute", left: LABEL_W + rowNowPx + 2, top:"50%", transform:"translateY(-50%)", zIndex:20 }}
                    className={stopBtnCls}>END</button>
                )}
                {rowCols.map(ci => {
                  ...
                  return (
                    <div key={ci} style={{ width: COL_W, minWidth: COL_W }}
                      ...
                      {seg && (
                        <>
                          ...
                          {(isActualStart || isRowCont) && (() => {
                            const visStart = Math.max(seg.startCol, colStart)
                            const visEnd   = Math.min(effectiveEnd, colEnd - 1)
                            const visW     = (visEnd - visStart + 1) * COL_W
                            return (
                      ...
                      {ci !== nowCol && seg && !isDragPreview && !seg.stopped && (
                        <button type="button" title="End fluid here" onClick={e => { e.stopPropagation(); extendFluid(seg.id, ci, true) }} className={hoverStopCls}>END</button>
                      )}
                      {stoppedSeg && (
                        <button type="button"
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <div className="ml-auto flex items-center gap-1.5">
          {(["expand", "scroll"] as const).map(mode => (
            <button key={mode} type="button" onClick={() => setLayout(mode)}
              className={`text-xs font-semibold px-3 py-1 rounded-full border transition-colors select-none ${
                layout === mode
                  ? "bg-slate-700 dark:bg-slate-200 border-slate-700 dark:border-slate-300 text-white dark:text-slate-900"
                  : "border-slate-200 dark:border-[#3a3a3a] text-slate-500 dark:text-[#888] hover:border-slate-400 dark:hover:border-[#555]"
              }`}>
              {mode === "expand" ? "Stacked" : "Scrollable"}
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="flex gap-3 items-start">
        ...
        <div className="flex-1 min-w-0 space-y-2">
          {layout === "expand"
            ...
            : (
              <div className="overflow-x-auto" onWheel={e => e.stopPropagation()}>
                <div style={{ minWidth: LABEL_W + colCount * COL_W }}>
          ...
          <div className="flex flex-wrap gap-x-3 gap-y-1 px-1">
            {([
              ["Click", "Select item"],
              ["Double-click stopped bar", "Reactivate"],
              ["Double-click infusion bar", "Change rate"],
              ["Double-click drug", "Change dose"],
              ["Del / Backspace", "Delete selected"],
              ["→", "Extend bar / copy drug right"],
              ["←", "Retract bar / remove drug"],
              ["Tab", "Cycle selection"],
              ["0-9", "Enter dose (drug)"],
              ["Esc", "Deselect"],
        ...
        {/* в"Ђв"Ђ Tags panel в"Ђв"Ђв"Ђв"Ђв"Ђв"Ђв"Ђв"Ђв"Ђв"Ђв"Ђв"Ђв"Ђв"Ђв"Ђв"Ђв"Ђв"Ђв"Ђв"Ђв"Ђв"Ђв"Ђв"Ђв"Ђв"Ђв"Ђв"Ђв"Ђв"Ђв"Ђв"Ђв"Ђв"Ђв"Ђв"Ђв"Ђв"Ђв"Ђв"Ђв"Ђв"Ђв"Ђв"Ђв"Ђв"Ђв"Ђв"Ђв"Ђв"Ђв"Ђв"Ђв"Ђв"Ђв"Ђв"Ђ */}
                ...
                <>
                  ...
                  <div className="flex gap-1.5">
                    ...
                    <select value={fp.unit} onChange={e => setFp(f => f ? {...f, unit:e.target.value} : f)}
                      ...
                      {["mg","mcg","mcg","ml","g","IU"].map(u => <option key={u}>{u}</option>)}
                    </select>
                  </div>
